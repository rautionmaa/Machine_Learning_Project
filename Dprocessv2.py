...
def dataProcess(dataframe):
    ...
    dataframe["PoolQC"] = dataframe["PoolQC"].apply(
        lambda s: mymap.get(s) if s in mymap else s).astype(int)
    dataframe['KitchenQual'] = dataframe['KitchenQual'] = dataframe["KitchenQual"].apply(
        lambda s: mymap.get(s) if s in mymap else s).astype(int)
    ...
    # dataframe["SaleCondition"] = dataframe.SaleCondition.replace({"Abnorml" : 0, "Alloca" : 0, "AdjLand" : 0, 
